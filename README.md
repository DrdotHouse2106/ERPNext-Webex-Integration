...
- Die Webex-*Organization-Contacts*-API liegt unter `{api_base}/v1/contacts/organizations/{orgId}/contacts/...`; `orgId` muss in Webex Settings hinterlegt sein. Sowohl der Pfad als auch das JSON-Schema (`displayName`, `firstName`, `lastName` direkt auf oberster Ebene, `phoneNumbers: [{value, type}]`) sind anhand echter Antwortdaten aus einem Live-Tenant verifiziert. Lediglich das genaue Verhalten beim **Anlegen/Aktualisieren** (POST/PUT) ist noch nicht live getestet – nach der Ersteinrichtung bitte über **"Vorschau"** und einen einzelnen Testeintrag prüfen.
