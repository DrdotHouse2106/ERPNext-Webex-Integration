...
- The Organization Contacts API lives at `{api_base}/v1/contacts/organizations/{orgId}/contacts/...`; `orgId` must be set in Webex Settings. Both the path and the JSON schema (`displayName`, `firstName`, `lastName` at the top level, `phoneNumbers: [{value, type}]`) have been verified against real response data from a live tenant. Only create/update (POST/PUT) behavior hasn't been live-tested yet — after initial setup, verify via **"Vorschau"** and a single test entry.
